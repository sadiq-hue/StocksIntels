  ...
  if (user?.role !== "admin") {
    return (
      <div className="max-w-xl mx-auto mt-16 text-center">
        ...
        <h2 className="text-xl font-bold text-foreground">Staff Access Only</h2>
        <p className="text-muted-foreground mt-2">You need admin privileges to access live chat management.</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-64px)] bg-muted">
      {/* Active conversations sidebar */}
      <div className="w-full lg:w-80 shrink-0 border-r border-border bg-card flex flex-col">
        <div className="p-4 border-b border-border">
          <h2 className="text-sm font-bold text-foreground flex flex-wrap items-center gap-2">
            ...
            Live Chat
            <span className="text-xs font-normal text-muted-foreground ml-auto">{activeChats.length} active</span>
          </h2>
        ...
        <ScrollArea className="flex-1">
          {loading ? (
            ...
            <div className="text-center py-12 px-4">
              ...
              <p className="text-sm text-muted-foreground">{filter ? "No matches" : "No active conversations"}</p>
                ...
                <button key={chat.user_id} onClick={() => setSelectedUserId(chat.user_id)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-semibold text-foreground truncate">{chat.user_name}</span>
                      <span className="text-[10px] text-muted-foreground shrink-0">{formatChatTime(chat.last_activity)}</span>
                    </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {!selectedUserId ? (
          ...
          <>
            {/* Conversation header */}
            <div className="px-5 py-3 border-b border-border bg-card shrink-0 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="size-9 rounded-full bg-[#0D7490]/10 flex items-center justify-center">
                  <User className="size-4 text-[#0D7490]" />
                </div>
                <div>
                  <p className="text-sm font-bold text-foreground">{selectedChat?.user_name || "User"}</p>
                ...
                <div className="space-y-3 py-4">
                  {[1,2,3].map(i => (
                    <div key={i} className="flex gap-3 animate-pulse">
                      ...
                      <div className="flex-1"><div className="h-4 bg-muted rounded w-1/4 mb-2" /><div className="h-12 bg-muted rounded-xl" /></div>
                    </div>
                  ))}
                </div>
              ) : messages.length === 0 ? (
                  ...
                  {messages.map(msg => (
                    <div key={msg.id} className={`flex gap-2.5 ${msg.is_staff ? "" : "flex-row-reverse"}`}>
                      <div className={`size-8 rounded-full flex items-center justify-center shrink-0 ${
                        msg.is_staff ? "bg-[#0D7490]/10" : "bg-muted"
                      }`}>
            ...
            <div className="p-4 border-t border-border bg-card shrink-0">
              <div className="flex flex-wrap items-center gap-2">
                <Input value={input} onChange={e => setInput(e.target.value)}
                  ...
                  placeholder="Type your reply..." className="h-11 bg-input border-2 border-border rounded-xl" />
