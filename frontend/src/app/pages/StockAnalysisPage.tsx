...
import {
  AreaChart, Area, BarChart, Bar, Line, ReferenceLine, XAxis, YAxis, CartesianGrid, Tooltip,
  ResponsiveContainer,
} from "recharts";
          ...
          <Card className="border shadow-sm overflow-hidden">
            <div className="p-6">
                            ...
                            <div className="bg-card border border-border rounded-lg shadow-lg p-3 text-xs space-y-1" style={{ fontSize: 12, minWidth: 160 }}>
                              ...
                              {d?.sma20 != null && (
                                <div className="flex justify-between gap-4 border-t border-border pt-1 mt-1">
                                  ...
                                  <span className="font-medium" style={{ color: '#f59e0b' }}>{d.sma20.toFixed(2)}</span>
                                </div>
                              )}
                              {d?.sma50 != null && (
                                <div className="flex justify-between gap-4">
                                  <span className="text-muted-foreground">SMA 50</span>
                                  <span className="font-medium" style={{ color: '#ef4444' }}>{d.sma50.toFixed(2)}</span>
                                </div>
                              )}
                            </div>
                          );
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-2 mt-4 pt-4 border-t border-border">
                ...
                <div className="flex items-center gap-2">
                  <div className="size-3 rounded-sm bg-[#0EA5E9]/40" />
                  <div>
                    <div className="text-[11px] font-medium text-muted-foreground">AVG VOLUME</div>
                    <div className="text-sm font-semibold text-foreground">
                      {(chartData.length > 0 ? (chartData.reduce((s: number, d: any) => s + d.volume, 0) / chartData.length / 1000000) : 0).toFixed(1)}M
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
