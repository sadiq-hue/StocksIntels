...
interface Signal {
  id: string; ticker: string; name: string; price: number; change: number;
  ...
  riskReward: number; confidence: number; timeframe: string;
  sector: string; volume: string; reason: string;
  country?: string;
  analysis?: {
    ...
    macro?: {
      score: number; grade: string; signal: string;
      country: string; summary: string;
...
export function SignalsPage() {
  ...
  useEffect(() => { fetchSignals(); const i = setInterval(fetchSignals, 300000); return () => clearInterval(i); }, []);
  ...
  if (loading && signals.length === 0) {
    return (
      <div className="p-4 md:p-6 max-w-[1400px] mx-auto flex items-center justify-center min-h-[60vh]">
        <div className="text-center"><RefreshCw className="w-8 h-8 text-[#0D7490] animate-spin mx-auto mb-4" /><p className="text-gray-500 text-sm">Loading trading signals...</p></div>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-gray-500 text-sm">AI-generated opportunities based on fundamental, technical & financial analysis</p>
        </div>
        <div className="flex items-center gap-3">
          {lastUpdated && <span className="text-gray-400 text-xs hidden sm:flex items-center gap-1"><Clock className="w-3 h-3" /> {lastUpdated}</span>}
          <Link to="/app/stocks">
            <Button variant="outline" size="sm" className="border-gray-200">
              <BarChart3 className="w-3.5 h-3.5 mr-2" />Screener
            </Button>
          </Link>
          <Button onClick={fetchSignals} disabled={loading} variant="outline" size="sm" className="border-gray-200">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        <Card className="bg-white border-gray-200 p-3"><p className="text-gray-400 text-[10px] uppercase tracking-wider">Total Signals</p><p className="text-gray-900 text-xl font-bold">{signals.length}</p></Card>
        <Card className="bg-white border-gray-200 p-3"><p className="text-gray-400 text-[10px] uppercase tracking-wider">Strong Buy/Buy</p><p className="text-emerald-600 text-xl font-bold">{strongBuy}</p></Card>
        <Card className="bg-white border-gray-200 p-3"><p className="text-gray-400 text-[10px] uppercase tracking-wider">Sell/Strong Sell</p><p className="text-red-600 text-xl font-bold">{strongSell}</p></Card>
        <Card className="bg-white border-gray-200 p-3"><p className="text-gray-400 text-[10px] uppercase tracking-wider">High Confidence</p><p className="text-gray-900 text-xl font-bold">{highConf.length}</p></Card>
        <Card className="bg-white border-gray-200 p-3">
          <p className="text-gray-400 text-[10px] uppercase tracking-wider">Avg Confidence</p>
          <p className="text-gray-900 text-xl font-bold">{signals.length ? Math.round(signals.reduce((a, b) => a + b.confidence, 0) / signals.length) : 0}%</p>
        </Card>
        <Card className="bg-white border-gray-200 p-3">
          <p className="text-gray-400 text-[10px] uppercase tracking-wider">Favorites</p>
        ...
        <div className="bg-gradient-to-r from-emerald-50 to-emerald-100/50 border border-emerald-200 rounded-lg p-4 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            {highConf.slice(0, 3).map(s => (
              <button key={s.ticker} onClick={() => { setSelected(s); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="px-2.5 py-1 bg-white border border-emerald-200 rounded-md text-xs font-medium text-emerald-700 hover:bg-emerald-50">{s.ticker} <span className="text-emerald-500">{s.confidence}%</span></button>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px] max-w-[260px]">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search ticker or name..." className="pl-9 h-9 text-sm border-gray-200" />
        </div>
        <Select value={filterType} onValueChange={setFilterType}>
          <SelectTrigger className="w-[120px] h-9 text-sm border-gray-200"><SelectValue placeholder="Type" /></SelectTrigger>
        ...
        <Select value={filterSignal} onValueChange={setFilterSignal}>
          <SelectTrigger className="w-[130px] h-9 text-sm border-gray-200"><SelectValue placeholder="Signal" /></SelectTrigger>
        ...
        <Select value={filterSector} onValueChange={setFilterSector}>
          <SelectTrigger className="w-[140px] h-9 text-sm border-gray-200"><SelectValue placeholder="Sector" /></SelectTrigger>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {paged.map(s => {
          ...
          const Icon = ss.icon;
          return (
            <Card key={s.id || s.ticker} className="bg-white border-gray-200 overflow-hidden hover:border-[#0D7490] hover:shadow-md transition-all cursor-pointer group" onClick={() => setSelected(s)}>
              ...
              <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <button onClick={e => { e.stopPropagation(); toggleFav(s.ticker); }} className="shrink-0 hover:text-amber-400 transition-colors">
                    <Star className={`w-4 h-4 ${favorites.includes(s.ticker) ? "fill-amber-400 text-amber-400" : "text-gray-300"}`} />
                  </button>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <Link to={`/app/stock/${s.ticker}`} onClick={e => e.stopPropagation()} className="text-sm font-bold text-gray-900 hover:text-[#0D7490]">{s.ticker}</Link>
              ...
              {/* Type + sector */}
              <div className="px-4 pb-3 flex items-center gap-2 text-[11px] text-gray-400">
              ...
              <div className="px-4 pb-3 flex items-center gap-3">
                <div className="flex-1 bg-gray-50 rounded-lg p-2.5 border border-gray-100">
                  ...
                  <p className="text-sm font-bold text-gray-900">${formatCurrency(s.price)}</p>
                </div>
              ...
              <div className="px-4 pb-3 flex items-center gap-3">
                ...
                <span className="text-xs text-gray-500">R:R 1:{s.riskReward.toFixed(1)}</span>
              </div>
              ...
              <div className="px-4 pb-4 pt-2 border-t border-gray-100">
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{s.reason}</p>
                </div>
              </div>
            </Card>
          );
      ...
      {paged.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          ...
          <p className="text-sm font-medium text-gray-500">No signals match your filters</p>
          <p className="text-xs mt-1 text-gray-400">Try adjusting your search or filter criteria</p>
        </div>
      )}

      {/* Pagination */}
      <div className="flex items-center justify-between border-t border-gray-200 pt-4">
        <span className="text-xs text-gray-400">{filtered.length} signal{filtered.length !== 1 ? "s" : ""}</span>
        <div className="flex items-center gap-1">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={safePage <= 1} className="flex items-center justify-center w-8 h-8 rounded-md border border-gray-200 bg-white hover:bg-gray-50 disabled:opacity-30 disabled:pointer-events-none transition-colors"><ChevronLeft className="w-4 h-4 text-gray-600" /></button>
          <span className="text-xs font-medium text-gray-500 px-3 min-w-[4rem] text-center">{safePage} / {totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={safePage >= totalPages} className="flex items-center justify-center w-8 h-8 rounded-md border border-gray-200 bg-white hover:bg-gray-50 disabled:opacity-30 disabled:pointer-events-none transition-colors"><ChevronRight className="w-4 h-4 text-gray-600" /></button>
        </div>
      </div>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl border border-gray-200" onClick={e => e.stopPropagation()}>
            <div className="p-6 space-y-5">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-0.5">
                    <h2 className="text-xl font-bold text-gray-900">{selected.ticker}</h2>
                ...
                <button onClick={() => setSelected(null)} className="p-1.5 hover:bg-gray-100 rounded-md transition-colors"><span className="text-gray-400 text-lg font-bold">&times;</span></button>
              </div>

              <div className="grid grid-cols-3 gap-3">
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-100"><p className="text-xs text-gray-400">Price</p><p className="text-lg font-bold text-gray-900">${formatCurrency(selected.price)}</p></div>
                <div className={`rounded-lg p-3 border ${selected.change >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}><p className="text-xs text-gray-400">Change</p><p className={`text-lg font-bold flex items-center gap-1 ${selected.change >= 0 ? "text-emerald-700" : "text-red-700"}`}>{selected.change >= 0 ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownRight className="w-4 h-4" />}{selected.change >= 0 ? "+" : ""}{selected.change}%</p></div>
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-100"><p className="text-xs text-gray-400">Confidence</p><p className={`text-lg font-bold ${selected.confidence >= 80 ? "text-emerald-600" : selected.confidence >= 70 ? "text-yellow-600" : "text-red-600"}`}>{selected.confidence}%</p></div>
              </div>

              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Trade Parameters</h3>
              ...
              {selected.analysis && (
                <>
                  {/* ── Why This Signal ── */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Why This Signal</h3>
                              ...
                              {grouped[cat].map(c => {
                                ...
                                return (
                                  <div key={c.key} className="flex items-start gap-2 text-xs">
                                    ...
                                    <span className="font-medium text-gray-700 min-w-[7rem]">{c.name}</span>
                                    <span className="text-gray-500 leading-tight">{c.rating}</span>
                                  </div>
                                );
                              })}
                            </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {Object.entries(selected.analysis.macro.conditions).map(([key, cond]) => {
                          ...
                          const style = sig === 'BUY' ? 'bg-emerald-50 border-emerald-200' :
                            sig === 'SELL' ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-100';
                          return (
                            <div key={key} className={`rounded-lg p-2.5 border ${style}`}>
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <span className={`text-[9px] font-bold px-1 py-0.5 rounded ${
                                  ...
                                  sig === 'SELL' ? 'bg-red-100 text-red-700' : 'bg-gray-200 text-gray-500'
                                }`}>{sig}</span>
                              </div>
                              <p className="text-[11px] text-gray-600 leading-tight">{(cond as any).detail}</p>
                            </div>
                          );
                        })}
                      </div>
              ...
              <div className="bg-[#0D7490]/5 rounded-lg p-4 border border-[#0D7490]/20">
                <div className="flex items-start gap-2">
                  ...
                  <div><p className="text-sm font-medium text-gray-900 mb-1">Signal Reason</p><p className="text-sm text-gray-600 leading-relaxed">{selected.reason}</p></div>
                </div>
              </div>
