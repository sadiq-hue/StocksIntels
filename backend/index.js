...
app.get('/api/admin/stocks/:id', async (req, res) => {
  try {
    ...
    // Attach fundamentals if table exists
    try {
      const f = await pool.query('SELECT pe_ratio, pb_ratio, market_cap, dividend_yield, roe FROM stock_fundamentals WHERE symbol = $1', [stock.ticker]);
      if (f.rows.length > 0) Object.assign(stock, f.rows[0]);
    } catch {}
    res.json(stock);
...
app.get('/api/admin/audit', async (req, res) => {
  try {
    ...
    const prevRevenue = parseFloat(prevPayments.rows[0].revenue);
    const activePaidCount = activePaid.rows[0].cnt;
    const allTimeUsers = totalUsers.rows[0].cnt;
    const allTimeRevenue = parseFloat(totalRevenue.rows[0].total);

    const retentionRate = activePaidCount > 0
      ? Math.round((1 - curr.churned / Math.max(1, activePaidCount + curr.churned)) * 100)
      : 100;
    const conversionRate = curr.signups > 0
      ? Math.round((curr.activeSubs / curr.signups) * 100)
      : 0;
    const arpu = curr.successfulPayments > 0
    ...
    const paymentByPlan = {};
    planBreakdown.rows.forEach(r => { paymentByPlan[r.plan_name || 'unknown'] = { count: r.count, revenue: parseFloat(r.revenue) }; });
    ...
    res.json({
      ...
      arpu,
      trends: {
        signups: calcTrend(curr.signups, prevSignups.rows[0].cnt),
        churned: calcTrend(curr.churned, prevChurned.rows[0].cnt),
        revenue: calcTrend(curr.revenue, prevRevenue),
        subAttempts: calcTrend(curr.subAttempts, prevSubAttempts.rows[0].total),
        payments: calcTrend(curr.successfulPayments, history.length > 1 ? history[history.length - 2]?.successfulpayments || 0 : 0),
      },
      history,
      paymentByProvider,
      paymentByPlan,
...
app.get('/api/financials/debug/:symbol', async (req, res) => {
  try {
    ...
    res.json({
      ...
      periodDates: (full?.data?.incomeStatementHistory || []).map(h => ({ date: h.date, pt: h.periodType, rev: h.revenue, totRev: h.totalRevenue, nii: h.netInterestIncome, ni: h.netIncome })),
      latestPeriod: full?.data?.incomeStatementHistory?.[0]?.period || null,
      latestRevenue: full?.data?.incomeStatementHistory?.[0]?.revenue || 0,
      latestTotalRevenue: full?.data?.incomeStatementHistory?.[0]?.totalRevenue || null,
...
app.get('/api/signals/sells', async (req, res) => {
  try {
    const audit = await getSellAudit();
...
// --- Stripe Checkout (card, sole proprietor — no business license) ---
app.post('/api/payments/stripe', async (req, res) => {
  try {
    const { amount, currency = 'USD', plan, userId, durationMonths } = req.body;
    if (!amount) return res.status(400).json({ error: 'Amount is required' });
    const planName = plan || 'Subscription';
    const externalRef = `STRIPE-${Date.now()}-${String(Math.random()).slice(2, 8)}`;
    ...
    res.json({ success: true, checkoutUrl: result.url, reference: externalRef });
  } catch (error) {
    console.error('[STRIPE] Checkout error:', error.message);
    res.status(500).json({ error: error.message || 'Failed to create Stripe checkout' });
  }
});
...
app.post('/api/payments/stripe-webhook', async (req, res) => {
  try {
    ...
    const raw = req.rawBody ? req.rawBody.toString('utf8') : JSON.stringify(req.body);
    const event = stripeService.verifyWebhook(raw, sig);
    if (!event) {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (reference && paid) {
        const txResult = await pool.query(
          `UPDATE payment_transactions SET status = 'success', callback_data = $1, updated_at = NOW()
           WHERE external_reference = $2 AND status = 'pending'
           RETURNING id, user_id, plan_name, duration_months, provider, amount, currency`,
          [JSON.stringify(event), reference]
        );
        const tx = txResult.rows[0];
        if (tx && tx.user_id) {
          ...
          console.log(`[STRIPE] Subscription activated: user=${tx.user_id} tier=${tier} months=${months}`);
          await awardCommission(tx.user_id, tier);
          try {
            const userRes = await pool.query('SELECT full_name, email FROM users WHERE id = $1', [tx.user_id]);
            const { full_name: uName, email: uEmail } = userRes.rows[0] || {};
          ...
          console.log(`[STRIPE] Payment confirmed but no user_id on transaction: ref=${reference}`);
        }
      }
    }
    res.json({ received: true });
...
async function sendDailyPortfolioReports() {
  try {
    ...
    if (users.length === 0) { console.log('[DAILY REPORT] No users with holdings or broker connections'); return; }
    console.log(`[DAILY REPORT] Generating reports for ${users.length} users...`);
    for (const user of users) {
      try {
        const { rows: brokers } = await pool.query(
          `SELECT bc.id,
                  bas.balance, bas.equity, bas.margin, bas.free_margin, bas.level,
                  bas.positions, bas.trade_history
           FROM broker_connections bc
           LEFT JOIN LATERAL (
             SELECT balance, equity, margin, free_margin, level, positions, trade_history
             FROM broker_account_snapshots
             WHERE broker_connection_id = bc.id
             ORDER BY snapshot_at DESC
             LIMIT 1
           ) bas ON true
           WHERE bc.user_id = $1`,
