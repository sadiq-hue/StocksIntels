...
async function sendDailySentimentEmail(email, data) {
  const {
    ...
    signals,
    nseContext, globalContext, corporateActions, macroData,
    aiNarrative, whatToWatch, globalIndices, kpiData
  } = data;
  ...
  function badgeClass(val) {
    if (!val) return 'badge-grey';
    const s = String(val).toUpperCase();
    if (s === 'BUY' || s === 'BULLISH' || s === 'BEAT' || s === 'DIVIDEND') return 'badge-green';
    if (s === 'SELL' || s === 'BEARISH' || s === 'MISS') return 'badge-red';
    if (s === 'WATCH' || s === 'NEUTRAL' || s === 'HOLD') return 'badge-amber';
...
<div style="max-width:640px;margin:32px auto;background:${CARD_WHITE};border-radius:8px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08)">
  ...
  <div style="padding:28px 32px;border-bottom:3px solid ${BRAND_COLOR};background:${sentimentHeroClass}">
    ...
    <div style="font-size:14px;color:${TEXT_LIGHT};margin-top:8px">Confidence: ${confidence || 'unavailable'} &nbsp;&middot;&nbsp; Based on price, volume, sentiment &amp; macro data</div>
  ...
  <div style="padding:22px 32px;border-bottom:1px solid ${BORDER}">
    <div style="font-size:11px;font-weight:700;color:${TEXT_MED};text-transform:uppercase;letter-spacing:0.8px;margin-bottom:14px;display:flex;align-items:center;gap:8px">Market Intelligence<span style="flex:1;height:1px;background:${BORDER};margin-left:8px"></span></div>
    <div style="display:flex;gap:10px;margin-bottom:4px">
      <div style="flex:1;border-radius:6px;padding:14px 12px;text-align:center;background:${BRAND_COLOR}">
        <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:6px;color:rgba(255,255,255,0.8)">Tracked Stocks</div>
      ...
      <div style="flex:1;border-radius:6px;padding:14px 12px;text-align:center;background:${GREEN}15">
        <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:6px;color:${GREEN}">Buy Ratings</div>
      ...
      <div style="flex:1;border-radius:6px;padding:14px 12px;text-align:center;background:${RED}15">
        <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:6px;color:${RED}">Sell Ratings</div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:18px">
      <thead>
        <tr style="background:${BRAND_COLOR}">
          ...
          ${nseGainers?.[0]?.signal || nseGainers?.[0]?.aiSignal ? `<th style="padding:9px 10px;text-align:right;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">AI Rating</th>` : ''}
        </tr>
      </thead>
      <tbody>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:18px">
      <thead>
        <tr style="background:${BRAND_COLOR}">
          ...
          <th style="padding:9px 10px;text-align:left;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">Sector</th>
          <th style="padding:9px 10px;text-align:right;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">Change</th>
          ${nseLosers?.[0]?.signal || nseLosers?.[0]?.aiSignal ? `<th style="padding:9px 10px;text-align:right;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">AI Rating</th>` : ''}
        </tr>
      </thead>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      <thead>
        <tr style="background:${BRAND_COLOR}">
          ...
          <th style="padding:9px 10px;text-align:right;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">Payment Date</th>
        </tr>
      </thead>
      <tbody>${corporateActionRows}</tbody>
    </table>` : ''}
  </div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:16px">
      <thead>
        <tr style="background:${BRAND_COLOR}">
          ...
          <th style="padding:9px 10px;text-align:right;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">Change</th>
          <th style="padding:9px 10px;text-align:right;font-size:11px;font-weight:600;color:#ffffff;letter-spacing:0.3px">YTD</th>
        </tr>
      </thead>
  ...
  <div style="background:${BRAND_COLOR};padding:28px 32px;text-align:center">
    <div style="color:rgba(255,255,255,0.8);font-size:13px;margin-bottom:16px">View live AI market intelligence and manage your portfolio on the StocksIntels platform.</div>
...
async function sendWeeklyDigestEmail(email, data) {
  ...
  const html = baseWrapper(`
    ...
    <div style="background:linear-gradient(135deg,${BRAND_COLOR}10,${BRAND_COLOR}05);border:1px solid ${BRAND_COLOR}30;border-radius:10px;padding:16px;margin-bottom:20px;text-align:center">
      ...
      <div style="font-size:12px;color:${TEXT_MED}">Tracked Stocks This Week</div>
    </div>` : ''}
...
async function sendDailyBriefEmail(email, data) {
  ...
  const html = baseWrapper(`
    ...
    <div style="text-align:center;margin-top:8px;border-top:1px solid ${BORDER};padding-top:16px">
      <div style="font-size:12px;color:${TEXT_LIGHT};margin-bottom:12px">Explore full market intelligence on the StocksIntels platform.</div>
...
function sendAnnouncementEmail(email, announcement, name) {
  ...
  return sendViaTransport({ to: email, subject, html, label: 'Announcement' });
}
