...
check('  buy entry carries stop/target/timestamp', m.get('X2').stopLoss === 95 && m.get('X2').target1 === 105 && m.get('X2').timestamp);
...
check('unresolved buy survives the next cycle unchanged', m.get('X4') && !m.get('X4').result && m.get('X4').stopLoss === 95);

console.log('── resolution clears the entry ──');

m = new Map(); st = state(); pf = perf();
trackSignalOutcomes(st, pf, m, 'X5', 100, buySig(), true);
trackSignalOutcomes(st, pf, m, 'X5', 94, holdSig(), true); // price <= stop, market open
check('stop-hit loss resolves and the entry is removed (fresh Hold not re-seeded)', m.size === 0);
...
trackSignalOutcomes(st, pf, m, 'X7', 94, holdSig(), false); // market closed -> defer
