...
// 18% MIN_STOP_PCT floor (95 -> 82 at entry 100), never leave it tight.
...
check('  sub-floor 5% stop is NORMALIZED to the 18% floor (95 -> 82)', m.get('X2').stopLoss === 82 && m.get('X2').target1 === 105 && m.get('X2').timestamp);
...
check('unresolved buy survives the next cycle unchanged (floored stop 82)', m.get('X4') && !m.get('X4').result && m.get('X4').stopLoss === 82);
...
// A pre-MIN_STOP_PCT position (like ASML's 2.61% stop) must be widened to the 18%
...
// does NOT stop out, and only a move below the floor (81 <= 82) resolves it.
m = new Map(); st = state(); pf = perf();
trackSignalOutcomes(st, pf, m, 'X11', 100, { signal: 'Buy', action: 'buy', stopLoss: 97, target1: 110, positionSize: '25%' }, true);
check('  legacy 3% stop normalized to the floor on seed (97 -> 82)', m.get('X11').stopLoss === 82);
advance(6 * 60 * 1000);
...
advance(6 * 60 * 1000);
trackSignalOutcomes(st, pf, m, 'X11', 80, holdSig(), true); // <= floored stop 82
